...
create view open_items as 
	select v.vendor_name, 
		i.invoice_id, 
		i.invoice_total , 
		(i.invoice_total - i.payment_total - i.credit_total) as "saldo devedor (balance_due)"	
	from vendors v      
		inner join 
	invoices i on 
...
AS
SELECT vendor_id, vendor_address1, 
vendor_address2, vendor_city, vendor_state, vendor_zip_code
FROM vendors;
select * from vendor_address;

### Exercício 14 ####
select * from vendor_address where vendor_id = 4;
